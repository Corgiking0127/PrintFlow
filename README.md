...
首次运行后，打开“打印机”页面，选择账号区域，填写拓竹账号、密码和设备序列号。保存时网页会自动配置同机 Adapter。
...
4. 如果拓竹要求邮箱验证，填写收到的验证码并再次保存。
5. 登录成功后 Adapter 会连接对应的 MQTT 节点并开始同步。

账号密码和验证码只用于当次 HTTPS 登录，不会保存到磁盘。登录成功后保存的是拓竹 Access Token。也可以在页面的高级选项直接输入已有 Token。
